...
import { ShoppingCartIcon, CheckCircleIcon, EyeIcon } from '../icons/Icons';
...
const PublicSiteRenderer: React.FC<PublicSiteRendererProps> = ({ store, onBack, onNewOrder }) => {
    ...
    const products = store.products; // Live products from store
    ...
    const renderBlock = (block: WebBlock) => {
        switch (block.type) {
            ...
            case 'text':
                return (
                    <div key={block.id} className="container mx-auto px-4 py-12 prose max-w-4xl text-center">
                        <p className="text-lg text-gray-600">{block.content.text}</p>
                    </div>
                );
            case 'product_grid':
                return (
                            ...
                            {products.slice(0, block.content.limit || 8).map(product => (
                                <div key={product.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition overflow-hidden flex flex-col">
                                    <div className="h-48 bg-gray-200 flex items-center justify-center text-gray-400">
                                        <EyeIcon /> {/* Placeholder image */}
                                    </div>
                                    <div className="p-4 flex-1 flex flex-col">
                                        <h3 className="font-bold text-gray-800 mb-1">{product.name}</h3>
                                        <p className="text-indigo-600 font-bold mb-4">{product.sellPrice.toLocaleString()} ج.م</p>
                                        <button 
                    ...
                    <div key={block.id} className="bg-gray-100 py-12">
                        <div className="container mx-auto px-4 max-w-lg">
                             ...
                             <form className="bg-white p-6 rounded-xl shadow-sm space-y-4" onSubmit={(e) => { e.preventDefault(); alert('شكرا لتواصلك معنا!'); }}>
                                 <input type="text" placeholder="الاسم" className="w-full p-3 border rounded" required />
                                 <input type="email" placeholder="البريد الإلكتروني" className="w-full p-3 border rounded" required />
                                 <textarea placeholder="رسالتك" className="w-full p-3 border rounded h-32" required></textarea>
            ...
            case 'image_carousel':
                return (
                    <div key={block.id} className="w-full overflow-hidden relative group">
                        ...
                        <div className="container mx-auto px-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                {(block.content.items || []).map((item: any, idx: number) => (
                                    <div key={idx} className="bg-white p-6 rounded-xl shadow-sm border border-indigo-100 text-center">
                ...
                return (
                    <div key={block.id} className="container mx-auto px-4 py-12 max-w-3xl">
                        ...
                        <div className="space-y-4">
                            {(block.content.items || []).map((item: any, idx: number) => (
                                <details key={idx} className="bg-white border rounded-lg group">
    ...
    return (
        <div className="min-h-screen bg-white font-sans relative" style={{ fontFamily: website.theme.fontFamily }}>
            ...
            <div className="fixed top-0 left-0 right-0 bg-gray-900 text-white p-2 z-50 flex justify-between items-center px-4 text-sm">
                ...
                <button onClick={onBack} className="bg-white text-gray-900 px-3 py-1 rounded hover:bg-gray-200">عودة للوحة التحكم</button>
            </div>

            {/* Website Header */}
            <header className="sticky top-[36px] z-40 bg-white/90 backdrop-blur-md border-b shadow-sm mt-[36px]">
                <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                    <div className="text-2xl font-bold text-gray-800">{website.title}</div>
                    <div className="flex items-center gap-6">
                        <nav className="hidden md:flex gap-6 text-gray-600">
                            <a href="#" className="hover:text-indigo-600 font-medium">الرئيسية</a>
                            <a href="#" className="hover:text-indigo-600 font-medium">المنتجات</a>
                            <a href="#" className="hover:text-indigo-600 font-medium">من نحن</a>
                        </nav>
                        <button 
            ...
            {isCheckoutOpen && (
                <div className="fixed inset-0 bg-black/50 z-[60] flex justify-end">
                    <div className="bg-white w-full max-w-md h-full shadow-2xl p-6 overflow-y-auto animate-fade-in-up">
                                        ...
                                        return (
                                            <div key={item.productId} className="flex justify-between items-center border-b pb-2">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-500">{item.quantity} x {p.sellPrice.toLocaleString()}</p>
                                                </div>
                                                <span className="font-bold">{(item.quantity * p.sellPrice).toLocaleString()}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                                
                                <div className="flex justify-between text-xl font-bold text-gray-800 border-t pt-4 mb-6">
                                    <span>الإجمالي</span>
                                    <span>{cartTotal.toLocaleString()} ج.م</span>
                                </div>

                                <form onSubmit={handleCheckout} className="space-y-4">
                                    <h3 className="font-bold text-gray-700">بيانات الشحن</h3>
                                    <input required type="text" placeholder="الاسم الكامل" className="w-full p-3 border rounded bg-gray-50" value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} />
                                    <input required type="tel" placeholder="رقم الهاتف" className="w-full p-3 border rounded bg-gray-50" value={customerInfo.phone} onChange={e => setCustomerInfo({...customerInfo, phone: e.target.value})} />
                                    <input required type="text" placeholder="العنوان بالتفصيل" className="w-full p-3 border rounded bg-gray-50" value={customerInfo.address} onChange={e => setCustomerInfo({...customerInfo, address: e.target.value})} />
                                    
                                    <div className="p-3 bg-yellow-50 border border-yellow-200 rounded text-sm text-yellow-800">
                                        الدفع عند الاستلام متاح حالياً فقط.
                                    </div>
